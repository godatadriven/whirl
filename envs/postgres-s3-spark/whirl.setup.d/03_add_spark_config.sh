...
wget http://central.maven.org/maven2/com/amazonaws/aws-java-sdk/1.7.4/aws-java-sdk-1.7.4.jar -O /usr/spark/jars/aws-java-sdk-1.7.4.jar && \
  echo "$AWS_SDK_CHECKSUM /usr/spark/jars/aws-java-sdk-1.7.4.jar" | sha256sum -c -

wget https://jdbc.postgresql.org/download/postgresql-42.2.5.jar -O /usr/spark/jars/postgresql-42.2.5.jar && \
  echo "$POSTGRES_JDBC_CHECKSUM /usr/spark/jars/postgresql-42.2.5.jar" | sha256sum -c -

wget http://central.maven.org/maven2/org/apache/hadoop/hadoop-aws/2.7.3/hadoop-aws-2.7.3.jar -O /usr/spark/jars/hadoop-aws-2.7.3.jar && \
