...
def _demo_s3_to_postgress(conn_id, templates_dict, **context):
    ...
    # pg_hook.insert_rows(templates_dict['target_table'], [tuple(r) for r in df.to_numpy()])
    # TODO create table upfront or find better way to insert DF
    ...
    pg_hook = PostgresHook(
        ...
        schema=templates_dict['target_schema'],
    )
...
s3_to_postgres = PythonOperator(
    ...
    templates_dict={
        ...
        "pg_conn_id": "local_pg",
        "target_schema": os.environ["POSTGRES_DB"],
        "target_table": "api",
    },
    provide_context=True,
    dag=dag
